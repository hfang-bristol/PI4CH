...
#'
#' \code{oDAGlevel} is supposed to calculate the level of nodes, given a direct acyclic graph (DAG; an ontology). The input is a graph of "igraph" or "graphNET" object, and the definition of the node level. The return can be the level for each node or the nodes for each level.
#'
...
#' @export
#' @seealso \code{\link{oDAGroot}}, \code{\link{oDAGreverse}}
#' @include oDAGlevel.r
#' @examples
...
#' oDAGlevel(subg, level.mode="longest_path")
#' # 4b) definition based on the shortest path
#' oDAGlevel(subg, level.mode="shortest_path")
#' # 4c) definition based on the longest path, and return nodes for each level
#' oDAGlevel(subg, level.mode="longest_path", return.mode="level2node")

oDAGlevel <- function (g, level.mode=c("longest_path","shortest_path"), return.mode=c("node2level","level2node"))
{
    ...
    if(level.mode=="longest_path"){
        edgelist <- as_data_frame(ig, what="edges")
