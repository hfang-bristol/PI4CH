...
#'
#' \code{oNetPipeline} is supposed to finish ab inito maximum-scoring subgraph identification for the input graph with the node information on the significance (p-value or fdr). It returns an object of class "igraph" or "graphNEL". 
#'
...
#' \itemize{
#' \item{ia) if the method is either "pdf" or "cdf", \code{\link{oBUMfit}} used to fit the p-value distribution under beta-uniform mixture model, and \code{\link{oBUMscore}} used to calculate the scores according to the fitted BUM and the significance threshold.}
#' \item{ib) if the method is either "customised", then the user input list of fdr (or p-values) and the significance threshold will be directly used for score transformation by \code{\link{oFDRscore}}.}
...
#' \item{iii) \code{\link{oNetFind}} used to find maximum-scoring subgraph from the input graph and scores imposed on its nodes.}
#' }
#' @export
#' @seealso \code{\link{oBUMfit}}, \code{\link{oBUMscore}}, \code{\link{oFDRscore}}, \code{\link{oNetFind}}
#' @include oBUMfit.r
#' @examples
#' \dontrun{
...
#' subgraph <- oNetPipeline(g=subg, pval=x, significance.threshold=0.1)
#' # 4b) assume the input is a list of customised significance (eg FDR directly)
#' subgraph <- oNetPipeline(g=subg, pval=x, method="customised", significance.threshold=0.1)
#' 
...
oNetPipeline <- function(g, pval, method=c("pdf","cdf","customised"), significance.threshold=NULL, nsize=NULL, plot=F, verbose=T)
{
    ...
    if(method!="customised"){
        ...
        if(plot){
            fit <- oBUMfit(pval, ntry=1, hist.bum=T, contour.bum=T, verbose=verbose)
        }else{
            fit <- oBUMfit(pval, ntry=1, hist.bum=F, contour.bum=F, verbose=verbose)
        }
    ...
    if(is.null(nsize)){
        ...
        for(i in 1:length(all_i)){
            ...
            if(method!="customised"){
                scores_test <- oBUMscore(fit=fit, method=method, fdr=fdr_test, scatter.bum=F)
            }else if(method=="customised"){
                scores_test <- oFDRscore(pval, fdr.threshold=fdr_test, scatter=F)
                if(is.null(scores_test)){
                	break
                }
            }
    
            module_test <- suppressWarnings(oNetFind(g, scores_test))
        ...
        if(nsize_rough==nsize){
        	...
        	for(i in 1:length(all_i)){
            	...
            	#########################################################
            	# Fix the error (temporarily 2024-04-12)
            	#########################################################
				if(is(suppressWarnings(try(module_test <- oNetFind(g, scores_test), TRUE)),"try-error")){
					break
				}else{
					#module_test <- suppressWarnings(oNetFind(g, scores_test))
					nsize_test <- vcount(module_test)
    ...
    if(method!="customised"){
        ...
        if(plot){
            scores <- oBUMscore(fit=fit, method=method, fdr=fdr_final, scatter.bum=T)
        }else{
            scores <- oBUMscore(fit=fit, method=method, fdr=fdr_final, scatter.bum=F)
        }
    }else if(method=="customised"){
        if(verbose){
        	now <- Sys.time()
        ...
        if(plot){
            scores <- oFDRscore(pval, fdr.threshold=fdr_final, scatter=T)
        }else{
            scores <- oFDRscore(pval, fdr.threshold=fdr_final, scatter=F)
        }
    }
    ####################
    ...
    subgraph <- oNetFind(g, scores)
    subgraph$threshold <- fdr_final
    if(verbose){
