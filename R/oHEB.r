...
oHEB <- function(g, leave.label.size=3, leave.label.color="black", leave.label.wrap=NULL, leave.label.expansion=1.1, leave.size=NULL, limit.expansion=1.2, edge.tension=0.8, edge.alpha=0.8, edge.width=0.5, edge.palette=NULL)
{
	...
	if(!all(c("community","name") %in% igraph::vertex_attr_names(ig))){
		...
		if(1){
			...
			df_tmp <- igraph::as_data_frame(ig,what="vertices")
			ls_tmp <- split(x=df_tmp$name, f=df_tmp$community)
			ls_degree <- lapply(ls_tmp, function(x){
				ig_tmp <- igraph::induced_subgraph(ig, x)
		...
		## adjacency relations
		##################
		df_edges <- igraph::as_data_frame(ig)
