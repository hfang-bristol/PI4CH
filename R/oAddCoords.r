...
#' @export
#' @seealso \code{\link{oAddCoords}}
#' @include oAddCoords.r
#' @examples
...
#' ig <- oAddCoords(g, node.attr="community", edge.color.alternative=c("grey50","grey95"))
...
oAddCoords <- function(g, node.attr=NULL, glayout=layout_with_kk, edge.color.alternative=c("grey70","grey95"), seed=825, verbose=TRUE)
{
	...
	df_nodes <- igraph::as_data_frame(ig,what="vertices")
	...
	if(1){
		...
		### edges
		df_edges <- igraph::as_data_frame(ig,what="edges")
