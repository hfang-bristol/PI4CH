...
oPierMatrix <- function(list_pNode, displayBy=c("score","rank","weight","pvalue","evidence"), combineBy=c('union','intersect'), aggregateBy=c("none","fishers","logistic","Ztransform","orderStatistic","harmonic","max","sum"), rangeMax=5, keep=FALSE, GR.Gene="UCSC_knownGene", verbose=TRUE, placeholder=NULL, guid=NULL)
{
	...
	if(displayBy=='evidence' | (displayBy=='pvalue' & aggregateBy!="none") | (displayBy=='score' & (aggregateBy %in% c("harmonic","max","sum")))){
		...
		if(0){
			...
			ls_edges <- pbapply::pblapply(list_pNode, function(x){
				relations <- igraph::as_data_frame(x$g, what="edges")
			})
			edges <- do.call(rbind, ls_edges) %>% unique()
			## get metag
			metag <- igraph::graph_from_data_frame(d=edges, directed=FALSE, vertices=nodes)
		}else{
			#############
			### very fast
			#############
			edges <- igraph::as_data_frame(list_pNode[[1]]$g, what="edges")
			## get metag
			metag <- igraph::graph_from_data_frame(d=edges, directed=FALSE, vertices=nodes)
		}
		##############
