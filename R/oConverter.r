...
#' @export
#' @seealso \code{\link{oConverter}}
#' @include oConverter.r
#' @examples
#' # generate a ring graph
#' g <- make_ring(10, directed=TRUE)
#' 
#' # convert the object from 'igraph' to 'dgCMatrix' class
#' oConverter(g, from='igraph', to='dgCMatrix')
#' 
#' \dontrun{
...
#' 
#' # convert the object from 'igraph' to 'dgCMatrix' class
#' s <- oConverter(g, from='igraph', to='dgCMatrix')
#' s[1:10,1:10]
#'
#' # convert the object from 'dgCMatrix' to 'igraph' class
#' ig <- oConverter(s, from="dgCMatrix", to="igraph")
#' ig
#' 
#' ##############
...
#' dtree <- oConverter(g, from='igraph', to='dtree')
#' lol <- oConverter(g, from='igraph', to='lol')
#' json <- oConverter(g, from='igraph', to='json')
#' 
#' # from "lol","json" to 'dtree' 
#' dtree <- oConverter(lol, from='lol', to='dtree')
#' dtree <- oConverter(json, from='json', to='dtree')
#' 
#' # from 'dtree' to "igraph"
#' g <- oConverter(dtree, from='dtree', to='igraph')
#' 
#' # force 'igraph' to a tree
#' gtree <- oConverter(g, from='igraph', to='igraph_tree')
#' }

oConverter <- function(obj, from=c("igraph","dgCMatrix","dtree","lol","json"), to=c("dgCMatrix","igraph","dtree","lol","json","igraph_tree"), verbose=TRUE)
{
    ...
    if(from=="igraph"){
        ...
        if(to=="dgCMatrix"){
        	...
        	df_edges <- igraph::as_data_frame(obj, what="edges")
				...
				if(to %in% c("lol","json")){
					...
					if(0){
						...
						func.igraph2list <- function(g, thisNode) {
							...
							childNodes <- V(g)[which(igraph::shortest_paths(g, thisNode, mode="out")==1)]
