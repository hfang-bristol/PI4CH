...
#' @export
#' @seealso \code{\link{oBiproject}}
#' @include oBiproject.r
#' @examples
...
oBiproject <- function(g, verbose=TRUE)
{
	...
	df_edges <- as_data_frame(ig, what="edges")[,c("from","to","weight")]
	...
	ig_x <- igraph::graph_from_adjacency_matrix(xM, mode="undirected", weighted=TRUE, diag=FALSE)
