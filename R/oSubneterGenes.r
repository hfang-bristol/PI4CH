...
oSubneterGenes <- function(data, network=NA, STRING.only=NA, network.customised=NULL, seed.genes=TRUE, subnet.significance=0.01, subnet.size=NULL, test.permutation=FALSE, num.permutation=100, respect=c("none","degree"), aggregateBy=c("Ztransform","fishers","logistic","orderStatistic"), verbose=TRUE, silent=FALSE, placeholder=NULL, guid=NULL)
{
	...
	if(is(subnet,"igraph")){
		relations <- igraph::as_data_frame(subnet, what="edges")[,c(1,2)]
		if(!is.null(subnet$combinedP)){
			relations$edgeConfidence <- igraph::as_data_frame(subnet, what="edges")[,"edgeConfidence"]
		}
		nodes <- igraph::as_data_frame(subnet, what="vertices")
		...
		if(igraph::is_directed(subnet)){
			subg <- igraph::graph_from_data_frame(d=relations, directed=TRUE, vertices=nodes)
		}else{
			subg <- igraph::graph_from_data_frame(d=relations, directed=FALSE, vertices=nodes)
		}
		if(!is.null(subnet$combinedP)){
