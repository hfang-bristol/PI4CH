...
#'
#' \code{oPierGenes} is supposed to prioritise genes given an input graph and a list of seed nodes. It implements Random Walk with Restart (RWR) and calculates the affinity score of all nodes in the graph to the seeds. The priority score is the affinity score. It returns an object of class "pNode". 
#'
#' @param data a named input vector containing a list of seed nodes (ie gene symbols; no less than 3 genes). For this named vector, the element names are seed/node names (e.g. gene symbols), the element (non-zero) values used to weight the relative importance of seeds. Alternatively, it can be a matrix or data frame with two columns: 1st column for seed/node names, 2nd column for the weight values
...
oPierGenes <- function(data, network=c("STRING_highest","STRING_high","STRING_medium","STRING_low","PCommonsUN_high","PCommonsUN_medium","PCommonsDN_high","PCommonsDN_medium","PCommonsDN_Reactome","PCommonsDN_KEGG","PCommonsDN_HumanCyc","PCommonsDN_PID","PCommonsDN_PANTHER","PCommonsDN_ReconX","PCommonsDN_TRANSFAC","PCommonsDN_PhosphoSite","PCommonsDN_CTD", "KEGG","KEGG_metabolism","KEGG_genetic","KEGG_environmental","KEGG_cellular","KEGG_organismal","KEGG_disease","REACTOME"), STRING.only=c(NA,"neighborhood_score","fusion_score","cooccurence_score","coexpression_score","experimental_score","database_score","textmining_score")[1], weighted=FALSE, network.customised=NULL, seeds.inclusive=TRUE, normalise=c("laplacian","row","column","none"), restart=0.7, normalise.affinity.matrix=c("none","quantile"), GR.Gene="UCSC_knownGene", verbose=TRUE, placeholder=NULL, guid=NULL)
{
    ...
    ##########
    if(is.vector(data)){
    	# however, this case NOT ALLOWED: named vector is exactly having two elements (1st Gene and 2nd weight)
		if(length(data)<=2){
			return(NULL)
		}
    }
    ...
    if(!is.null(network.customised) && is(network.customised,"igraph")){
		...
		#####################################################
		# if network.customised being a directed graph
		# internally converted into an undirected graph
		#####################################################
		if(igraph::is_directed(network.customised)){
			g <- igraph::as.undirected(network.customised)
		}else{
		...
		g <- oDefineNet(network=network, STRING.only=STRING.only, weighted=weighted, verbose=FALSE, placeholder=placeholder, guid=guid)
	
	}
    ######################################################################################
    
    pNode <- oPier(seeds=data, g=g, seeds.inclusive=seeds.inclusive, normalise=normalise, restart=restart, normalise.affinity.matrix=normalise.affinity.matrix, verbose=verbose)
    ...
    if(is(pNode,"pNode")){
		...
		df_nodes <- igraph::as_data_frame(pNode$g, what="vertices")[,c("name","description")]
    ...
    if(verbose){
		...
		message(paste(c("Runtime in total (oPierGenes): ",runTime," secs\n"), collapse=""), appendLF=TRUE)
	}
