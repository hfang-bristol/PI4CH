...
oRWR <- function(g, normalise=c("laplacian","row","column","none"), setSeeds=NULL, restart=0.75, normalise.affinity.matrix=c("none","quantile"), verbose=TRUE)
{
    ...
    #nig <- graph_from_adjacency_matrix(nadjM, mode=c("undirected"), weighted=TRUE, diag=FALSE, add.colnames=NULL, add.rownames=NA)
    ...
    if(verbose){
		...
		message(paste(c("Runtime in total (oRWR): ",runTime," secs\n"), collapse=""), appendLF=TRUE)
	}
