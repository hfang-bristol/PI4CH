...
#' subgraph <- oNetFind(subg, scores)

oNetFind <- function(g, scores)
{
    ...
    if(length(pos.nodes)==0){
        ...
        pos.subgraph <- oNetInduce(ig, pos.nodes, knn=0, remove.loops=T, largest.comp=F, verbose=F)
        ...
        edgelist <- igraph::as_edgelist(ig, names=F)
        ...
        if(length(mst.cluster.id)==1){
            ...
            for (j in 1:(length(mst.cluster.id)-1)) {
                ...
                path <- igraph::all_shortest_paths(mst, from=mst.cluster.id[j], to=mst.cluster.id[(j+1):length(mst.cluster.id)])
                all.ids <- c(all.ids, unlist(path$res))
            }
            all.ids <- unique(all.ids)
            
            sub.mig <- oNetInduce(mig, V(mst)[all.ids]$name, knn=0, remove.loops=T, largest.comp=F, verbose=F)
        ...
        if (length(neg.node.ids.2) == 0) {
            ...
            tmp_nodes <- unlist(lapply(conn.comp.graph, igraph::vertex_attr,"name")[ttmp])
